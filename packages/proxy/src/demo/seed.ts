...
import defaultPolicies from "../policy/defaultPolicies.json" with { type: "json" };
import { evaluatePolicy } from "../policy/engine.js";
import type { PolicyRule } from "../policy/types.js";

/**
 * Reusable demo seed (spec §14 milestone 2 + 3, AG-11):
 ...
 *
 * Used by both `npm run seed` (CLI) and POST /api/demo/reset (dashboard).
 */
...
async function seedDefaultPolicies(): Promise<PolicyRule[]> {
  ...
  for (const seed of seeds) {
    ...
    upserted.push(row as unknown as PolicyRule);
  }
  return upserted;
}
...
/**
 * Idempotent demo seed. Safe to call from the CLI or the reset endpoint —
 * clears requests/audit, upserts admin + policies, then re-seeds history.
 */
export async function runSeed(): Promise<SeedSummary> {
  ...
  const created = await prisma.toolCallRequest.createManyAndReturn({
    data: fakeRequests.map((r) => {
      ...
      return {
        ...
        toolName: r.toolName,
        toolInput: r.toolInput as Prisma.InputJsonValue,
        status: r.status,
        reasoning: r.reasoning,
        result: r.result as Prisma.InputJsonValue | undefined,
  ...
  const statusMix = created.reduce<Record<string, number>>((acc, r) => {
